...
#define PROGRAM_NAME		"pigeonminer"
...
static bool is_dev_time() {
	...
	if (dev_portion < 12) // No point in bothering with less than 10s
		return false;
	return (time(NULL) - dev_timestamp) % DONATE_CYCLE_TIME >= (DONATE_CYCLE_TIME - dev_portion);
}
	...
	while (!abort_flag) {
		...
		if (!wanna_mine(thr_id))
		{
			...
			if (!pool_is_switching) {
				...
				if (!opt_quiet)
					applog(LOG_WARNING, "Switching to Dev pool timed out...");
				if (!thr_id) pools[cur_pooln].wait_time += 1;
				pool_is_switching = false;
			}
			sleep(1);
...
int main(int argc, char *argv[])
{
	...
	printf("*** Pigeonminer " PACKAGE_VERSION " for nVidia GPUs ***\n");
	printf("***  optimized ccminer based on  by tpruvot@github  ***\n");
	printf("***          https://www.pigeoncoin.org             ***\n");
	...
	if (!opt_quiet) {
		...
		printf("PGN donation address: PDiZ89gk5HMqwrcGp6Hs9WdgFALzLbD4HG (Pigeoncoin)\n");
		printf("BTC donation address: 1NaVP4cKiWY6MxSDkTCZ2kh5Xm3coA27Qv (Pigeoncoin)\n\n");
	}
	...
	if (dev_donate_percent == 0.0) {
		printf("No dev donation set. Please consider making a donation to the following addresses:\n");
		printf("BTC donation address: 1NaVP4cKiWY6MxSDkTCZ2kh5Xm3coA27Qv (pigeoncoin)\n");
		printf("PGN donation address: PDiZ89gk5HMqwrcGp6Hs9WdgFALzLbD4HG (pigeoncoin)\n\n");
	}
	else {
		...
		strcpy(rpc_user, "PDiZ89gk5HMqwrcGp6Hs9WdgFALzLbD4HG.donate");
		strcpy(rpc_pass, "x");
		strcpy(rpc_url, "stratum+tcp://pool.pigeoncoin.org:3663");
